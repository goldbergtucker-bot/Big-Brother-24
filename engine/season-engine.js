...
(function(){
  ...
  function selectPOVPlayers(s,week){
    ...
    let pool;
    if(s.nomineeGroupId && noms.length>2){
      pool=living(s).slice();
    }else if(s.nomineeGroupId){
      const otherGroups=s.bestieGroups.filter(g=>g.id!==s.nomineeGroupId&&g.memberIds.some(id=>{const p=hg(s,id);return p&&p.active&&p.id!==hoh.id&&!noms.some(n=>n.id===p.id);}));
      const extraGroup=pick(otherGroups);
      const extraMembers=extraGroup?extraGroup.memberIds.map(id=>hg(s,id)).filter(p=>p&&p.active&&!noms.some(n=>n.id===p.id)&&p.id!==hoh.id):[];
      pool=[hoh,...noms,...extraMembers];
    }else{
      const excludeIds=backstageExcludedIds(s,week);
    ...
    log(s,{week,phase:s.phase,type:"pov-players",hohId:hoh.id,nomineeIds:s.nominees,povPlayers:s.povPlayers,participants:s.povPlayers,title:"POV Picked Players",lines:[s.nomineeGroupId&&noms.length>2?"A trio or larger is nominated, so the six-player cap is waived and every houseguest plays for Veto.":`${displayName(hoh)} and the nominees are automatically selected; the remaining slots are randomly drawn.`]});
    return pool;
  }
